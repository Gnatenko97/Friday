...
def RISE_1D(img, model, class_index, N_MASKS=8000, W=224,C=3):
    '''
	img: a 2-D input image (W samples, C channels)
	model: a trained model
	class_index; The class of interest
	N_MASKS: The number of random masks to be generated
	W,C: The desired dimensions of the random masks
	'''
    X = np.zeros(shape=(N_MASKS, W, C), dtype=np.float32)
    masks = np.zeros((N_MASKS,W,1), dtype=np.float32)
    #for i in tqdm(range(N_MASKS)):
    for i in range(N_MASKS):
        m =create_random_mask_1D(W=W)
        masks[i] = m
        x = img.copy()
...
def create_random_mask_1D(w=7, W=224, p_1=0.5, resample=Image.BILINEAR):
    '''
    Generates one random mask utilized in RISE
    inputs:
        h, w: initial size of binary mask
    ...
    assert W>w, 'Masks should be resized to higher dimensions.'
    # create random binary hxw mask
    mask=np.random.choice([0, 1], size=(W,1), p=[1-p_1, p_1])

#     # upsample mask to (h+H,w+W)
#     mask = Image.fromarray(mask*255.)
#     mask = mask.resize((W + w), resample=resample)
#     mask = np.array(mask)

#     # randomly crop mask to HxW
#     w_crop = np.random.randint(0,w+1)
#     mask = mask[w_crop:W + w_crop]
